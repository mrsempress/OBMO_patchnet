...
class FPointNet(nn.Module):
    def __init__(self, cfg, num_heading_bin, num_size_cluster, mean_size_arr, obmo=False):
        super().__init__()
        self.cfg = cfg
        ...
        if not self.obmo:
            ...
            self.box_est_head = nn.Sequential(nn.Linear(515, 512), nn.BatchNorm1d(512), nn.ReLU(inplace=True),
                                          nn.Linear(512, 256), nn.BatchNorm1d(256), nn.ReLU(inplace=True),
                                          nn.Linear(256, 3 + self.num_heading_bin*2 + self.num_size_cluster*4+1))
    ...
    def forward(self, point_cloud, one_hot_vec):
        ...
        output_dict = parse_outputs(box_results, self.num_heading_bin, self.num_size_cluster, self.mean_size_arr, output_dict, self.obmo)
