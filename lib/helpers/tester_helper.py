...
class Tester(object):
    ...
    def test(self):
        load_checkpoint(model=self.model,
                        ...
                        logger=self.logger)
        _2Dto3D = self.cfg['ocm3d'] if self.cfg.__contains__('split') else False
        split = self.cfg['split'] if self.cfg.__contains__('split') else ""
        self.test_one_epoch(_2Dto3D, split)


    def test_one_epoch(self, _2Dto3D, split):
