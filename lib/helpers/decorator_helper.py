...
def decorator(model, data, type,
                config = {'corner_loss_weight': 10.0, 'box_loss_weight': 1.0, 'quality_score_loss_weight': 0.0},
                obmo=False,
            ):
    '''
    :param model:
    ...
    if not obmo:
        ...
        if type == 'fpointnet':
            points, seg, center, angle_class, angle_residual, size_class, size_residual,\
                                                                rot_angle, one_hot_vec, quality_score = data
            output_dict = model(points, one_hot_vec)
            loss = fpointnet_loss(seg, center, angle_class, angle_residual, size_class, size_residual,
                            model.num_heading_bin, model.num_size_cluster, model.mean_size_arr, output_dict, quality_score,
                            config['corner_loss_weight'], config['box_loss_weight'], config['quality_score_loss_weight']
                            )

        elif type == 'patchnet':
            patch, center, angle_class, angle_residual, size_class, size_residual, rot_angle, one_hot_vec, quality_score = data
            output_dict = model(patch, one_hot_vec)
            loss = patchnet_loss(center, angle_class, angle_residual, size_class, size_residual,
                                model.num_heading_bin, model.num_size_cluster, model.mean_size_arr, output_dict, quality_score,
                                config['corner_loss_weight'], config['box_loss_weight'], config['quality_score_loss_weight']
